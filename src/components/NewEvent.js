				...
				<div className="flex flex-col h-full space-y-4 mt-6 p-4 ">
					<label className="flex flex-col w-full max-w-md">
						...
						<input
							ref={titleRef}
							type="text"
							className="input w-full max-w-md bg-base-100  hover:placeholder:text-black"
							value={title}
					...
					<label className="flex flex-col w-full max-w-md">
						...
						<input
							ref={locationRef}
							type="text"
							className="input w-full max-w-md  hover:placeholder:text-black"
							value={location}
					...
					<label className="flex flex-col w-full max-w-md">
						Starting Time:
						<select
							className="select select-bordered w-full max-w-md hover:placeholder:text-black"
					...
					<label className="flex flex-col w-full max-w-md">
						<span className="input w-full max-w-md flex flex-row justify-between items-center rounded-md mt-6 hover:placeholder:text-black">
							...
							<input
								...
								value={privateEvent}
								onChange={(e) => setPrivateEvent(!privateEvent)}
							/>
						</span>
					</label>
				</div>
...
function addNewEvent(fields, alert, setAlert) {
	console.log("AAAAAAAAAAA" + fields.privateEvent)
	try {
		...
		if (Number(fields.startTime) > Number(fields.endTime)) {
			console.log("dio cane " + fields.startTime + " ; " + fields.endTime)
			throw new Error("Start time must be before end time")
		}

		const d = new Date(fields.date).toDateString()
		const argsObj = fields
		argsObj.date = d
