...
const getEventsbyQuery = async (query) => {
	let events = []
	if (defaultOP.includes(query.op) == false) {
		throw new Error("invalid operator")
	}
	const q = query(collection(db, "events"), where(query.key, query.op, query.value))
...
const getEventsbyUid = async (uid) => {
	let events = []
...
async function addFollow(follow) {
	...
	if (userFiles.length != 1) throw new Error("user selection error")

	if (!userFiles[0].data.follow) userFiles[0].data.follow = []
	if (!userFiles[0].data.follow.includes(follow)) userFiles[0].data.follow.push(follow)
	else throw new Error("already following")

	console.log(userFiles[0].data)

	await updateDoc(doc(db, "users", userFiles[0].id), userFiles[0].data, {
		merge: true,
	})
}
...
async function removeFollow(unfollowId) {
	...
	if (userFiles.length <= 0) throw new Error("no follows")

	if (!userFiles.includes(unfollowId)) throw new Error("not following")
...
export { findUsersByIdList, removeFollow, getEventsbyUid, getEventsbyDate }
